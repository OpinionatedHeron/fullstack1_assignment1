...
suite("Location Model tests", () => {
  ...
  setup(async () => {
    ...
    for (let i = 0; i < testLocations.length; i += 1) {
      ...
      testLocations[i] = await db.locationStore.addLocation(
        favesList._id,
        testLocations[i]
      );
    }
  ...
  test("get multiple locations", async () => {
    const locations = await db.locationStore.getLocationsByFolderId(
      favesList._id
    );
  ...
  test("delete One location - success", async () => {
    ...
    const deletedLocation = await db.locationStore.getLocationById(
      testLocations[0]._id
    );
